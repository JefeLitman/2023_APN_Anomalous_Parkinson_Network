"""This file contains the loop for train and eval mode for GANomaly model.
...
def exec_loop(opts, readme_template, kfold, TP, TN, FP, FN, AUC, gen_loss, disc_loss, train_data, test_data):
    """This function execute the loop for training and evaluation in each epoch for GANomaly model. It doesn't return anything but it will be showing the results obtained in each epoch.
    Args:
    ...
    for epoch in range(opts["epochs"]):
        ...
        for step, xyi in enumerate(test_data):
            ...
            # Save the latent vectors, videos and errors in the last epoch and every 500 epochs
            if epoch % 1000 == 0 or epoch + 1 == opts["epochs"]:
                save_model_results(xyi, fake_images, latent_i, latent_o, feat_real, feat_fake, outputs_path, False, step == 0)
