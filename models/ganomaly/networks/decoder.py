"""This file contains the decoder network used in GANomaly net translated to Tensorflow.
...
def get_decoder(x, isize, nz, nc, ngf, n_extra_layers=0):
    """Function that return the layers of Decoder for GANomaly model translated in tensorflow.
    Args:
        ...
        nc (Integer): Quantity of channels to output.
    ...
    x_prime = tf.keras.layers.Conv2DTranspose(filters=cngf, kernel_size=4, strides=1, 
    ...
    csize, _ = 4, cngf
    while csize < isize // 2:
        x_prime = tf.keras.layers.Conv2DTranspose(filters=cngf//2, kernel_size=4, strides=2, 
    ...
    for t in range(n_extra_layers):
        x_prime = tf.keras.layers.Conv2DTranspose(filters=cngf, kernel_size=3, strides=1, 
            padding="same", use_bias=False, kernel_initializer=conv_kernel_init(),
    ...
    x_prime = tf.keras.layers.Conv2DTranspose(filters=nc, kernel_size=4, strides=2, 
        padding="same", use_bias=False, kernel_initializer=conv_kernel_init(),
