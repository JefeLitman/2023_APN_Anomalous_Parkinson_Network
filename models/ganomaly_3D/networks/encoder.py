"""This file contains the encoder network used in GANomaly 3D net in Tensorflow.
Version: 1.1
...
def get_encoder(x, isize, nz, nc, ndf, n_extra_layers=0, encoder_id=0, add_final_conv=True):
    """Function that add and forward the layers of Encoder for GANomaly 3D model in tensorflow. This method return two variables, the classifier output and features of the network
    Args:
        ...
        ndf: Quantity of initial filters in the first convolution (Integer).
        n_extra_layers: Quantity of layer blocks to add before reduction (Integer).
        encoder_id: Number id to make encoder unique among all enconders (Integer).
        add_final_conv: Add the final convolution with nz filters and unitary dimensions (Boolean).
    """
    if isize % 16 != 0:
        raise ValueError("isize has to be a multiple of 16")

    x_prime = tf.keras.layers.Conv3D(filters=ndf, kernel_size=4, strides=2, 
    ...
    for t in range(n_extra_layers):
        x_prime = tf.keras.layers.Conv3D(filters=cndf, kernel_size=3, strides=1, 
            padding="same", use_bias=False, kernel_initializer=conv_kernel_init(),
    ...
    while csize > 4:
        in_feat = cndf
        out_feat = cndf * 2
        x_prime = tf.keras.layers.Conv3D(filters=out_feat, kernel_size=4, strides=2, 
            padding="same", use_bias=False, kernel_initializer=conv_kernel_init(),
    ...
    if add_final_conv:
        classification = tf.keras.layers.Conv3D(filters=nz, kernel_size=4, strides=1, 
