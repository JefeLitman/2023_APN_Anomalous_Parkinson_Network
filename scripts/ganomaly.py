...
def run(mode):
    """Function that is similar to the notebook of GANomaly model, and it lets you run the model directly from the scripts, you only need to call this method in the run.py file specifying one mode to run the model.
    Args:
    ...
    """
    print("Starting Ganomaly model execution...\nLoading hiperparameters...")
    from models.ganomaly.hiperparameters import get_options
    opts = get_options()
    ...
    from models.ganomaly.modes.train import exec_loop as train
    from models.ganomaly.modes.train_eval import exec_loop as train_eval
